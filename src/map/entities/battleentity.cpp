...
int16 CBattleEntity::GetWeaponDelay(bool tp)
{
    ...
    if (auto weapon = dynamic_cast<CItemWeapon*>(m_Weapons[SLOT_MAIN]))
    {
        ...
        if (!tp)
        {
            ...
            {
                if (mobEntity->getMobMod(MOBMOD_ATTACK_SKILL_LIST) != 0)
                {
...
int16 CBattleEntity::addTP(int16 tp)
{
    ...
    if (PEffect)
    {
        ...
        {
            int16 cap = std::clamp(health.tp + tp, 0, max);
            tp = health.tp - cap;
            health.tp = cap;
        }
    }
...
void CBattleEntity::OnCastFinished(CMagicState& state, action_t& action)
{
    ...
    {
        if (this->objtype == TYPE_MOB && (PActionTarget->objtype == TYPE_PC || PActionTarget->objtype == TYPE_TRUST))
        {
...
void CBattleEntity::OnCastInterrupted(CMagicState& state, action_t& action, MSGBASIC_ID msg, bool blockedCast)
{
    ...
    if (PSpell)
    {
        ...
        else
        {
            ...
            // For some reason, despite the system supporting interrupted message in the action packet (like auto attacks, JA), a 0x029 message is sent for
            // spells.
...
bool CBattleEntity::OnAttack(CAttackState& state, action_t& action)
{
    ...
    while (attackRound.GetAttackSwingCount() && !(PTarget->isDead()))
    {
        ...
        {
            uint16 zanshinChance = this->getMod(Mod::ZANSHIN) + this->getMod(Mod::HASSO_SEIGAN_GIFT) + battleutils::GetMeritValue(this, MERIT_ZASHIN_ATTACK_RATE);
