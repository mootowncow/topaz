...
namespace charutils
{
    ...
    void	AddExperiencePoints(bool expFromRaise, CCharEntity* PChar, CBaseEntity* PMob, uint32 exp, EMobDifficulty mobCheck = EMobDifficulty::TooWeak, bool isexpchain = false, bool isSilent = false, bool xpOnly = false);
    ...
    uint8   AddItemTreasure(CCharEntity* PChar, uint8 LocationID, uint16 itemID, uint32 quantity = 1, bool silence = false, uint8 appraisalID = 0);
    ...
    void    DeleteSingleItemRank(CCharEntity* PChar, CItemEquipment* item);   // Deletes a single item rank for Rank Point system
