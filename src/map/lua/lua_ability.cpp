...
int32 CLuaAbility::getTotalTargets(lua_State* L)
{
    TPZ_DEBUG_BREAK_IF(m_PLuaAbility == nullptr);
...
int32 CLuaAbility::getPrimaryTargetID(lua_State* L)
{
    TPZ_DEBUG_BREAK_IF(m_PLuaAbility == nullptr);

    lua_pushinteger(L, m_PLuaAbility->getPrimaryTargetID());
    return 1;
}

int32 CLuaAbility::getCE(lua_State* L)
{
    TPZ_DEBUG_BREAK_IF(m_PLuaAbility == nullptr);

    lua_pushinteger(L, m_PLuaAbility->getCE());
    return 1;
}
...
inline int32 CLuaAbility::setFlag(lua_State* L)
{
    TPZ_DEBUG_BREAK_IF(m_PLuaAbility == nullptr);
    TPZ_DEBUG_BREAK_IF(lua_isnil(L, -1) || !lua_isnumber(L, -1));

    m_PLuaAbility->setFlag(lua_tointeger(L, -1));
    return 0;
}
...
inline int32 CLuaAbility::hasMissMsg(lua_State* L)
{
    ...
    lua_pushboolean(L, m_PLuaAbility->hasMissMsg());
    return 1;
}
...
Lunar<CLuaAbility>::Register_t CLuaAbility::methods[] =
{
    ...
    LUNAR_DECLARE_METHOD(CLuaAbility,getPrimaryTargetID),
