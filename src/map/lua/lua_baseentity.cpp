...
inline int32 CLuaBaseEntity::PrintToArea(lua_State* L)
{
    ...
    {
        message::send(MSG_CHAT_UNITY, nullptr, 0, new CChatMessagePacket(PChar, messageLook, (char*)lua_tostring(L, 1), name));
...
inline int32 CLuaBaseEntity::getTPToAttacker(lua_State* L)
{
    ...
    int16 baseTp = battleutils::GetBaseTP(PAttacker, weaponSlot);
    int16 tpToGain = (int16)(tpMultiplier * (baseTp * (1.0f + 0.01f * (float)((PAttacker->getMod(Mod::STORETP) + battleutils::getStoreTPbonusFromMerit(PAttacker))))));
...
inline int32 CLuaBaseEntity::addMaxRecastToAllAbilities(lua_State* L)
{
    ...
    if (PChar && PChar->objtype == TYPE_PC)
    {
        ...
        if (!lua_isnil(L, 2) && lua_isboolean(L, 1))
        {
            reset2hrs = lua_toboolean(L, 1);
        }
 ...
 *  Notes   : Calls the DoRandomDealToEntity member of battleutils
 ************************************************************************/
...
inline int32 CLuaBaseEntity::doRandomDeal(lua_State* L)
{
    ...
    CLuaBaseEntity* PEntity = Lunar<CLuaBaseEntity>::check(L, 1);
    lua_pushboolean(L,battleutils::DoRandomDealToEntity(static_cast<CCharEntity*>(m_PBaseEntity), static_cast<CCharEntity*>(PEntity->m_PBaseEntity)));
...
inline int32 CLuaBaseEntity::magicDmgTaken(lua_State *L)
{
    ...
    if (!lua_isnil(L, 2) && lua_isnumber(L, 2) && lua_tointeger(L, 2) > 0 && lua_tointeger(L, 2) < 9)
        element = (ELEMENT)lua_tointeger(L, 2);

    if (!lua_isnil(L, 3) && lua_isnumber(L, 3))
        rawDmg = lua_tointeger(L, 3);

    if (!lua_isnil(L, 4) && lua_isboolean(L, 4))
        IsCovered = lua_toboolean(L, 4);

    lua_pushinteger(L, battleutils::MagicDmgTaken((CBattleEntity*)m_PBaseEntity, dmg, element, rawDmg, IsCovered));
    return 1;
}
...
inline int32 CLuaBaseEntity::breathDmgTaken(lua_State* L)
{
    ...
    lua_pushinteger(L, battleutils::BreathDmgTaken((CBattleEntity*)m_PBaseEntity, damage, (ELEMENT)element, rawDmg, IsCovered));
    return 1;
}
...
inline int32 CLuaBaseEntity::getRangedDelay(lua_State* L)
{
    ...
    lua_pushinteger(L, (int16)((CBattleEntity*)m_PBaseEntity)->CBattleEntity::GetRangedWeaponDelay(tp));
    return 1;
}
...
inline int CLuaBaseEntity::clearSpellList(lua_State* L)
{
    TPZ_DEBUG_BREAK_IF(m_PBaseEntity == nullptr);
    TPZ_DEBUG_BREAK_IF(m_PBaseEntity->objtype != TYPE_MOB && m_PBaseEntity->objtype != TYPE_PET);
...
inline int32 CLuaBaseEntity::setMobMod(lua_State *L)
{
    ...
    {
        // this once broke on an entity (17532673) but it could not be found
        ShowError("CLuaBaseEntity::setMobMod Expected type mob or trust (%d) but its a (%d)\n", m_PBaseEntity->id, m_PBaseEntity->objtype);
        return 0;
    }
...
{
    TPZ_DEBUG_BREAK_IF(m_PBaseEntity == nullptr);
    TPZ_DEBUG_BREAK_IF(m_PBaseEntity->objtype == TYPE_NPC || m_PBaseEntity->objtype == TYPE_SHIP);
    TPZ_DEBUG_BREAK_IF(lua_isnil(L, 1) || !lua_isnumber(L, 1));
