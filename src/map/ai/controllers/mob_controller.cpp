...
bool CMobController::MobSkill(int wsList)
{
    ...
    for (auto skillid : skillList)
    {
        ...
        if (luautils::OnMobSkillCheck(PActionTarget, PMob, PMobSkill) == 0) // A script says that the move in question is valid
        {
            if (currentDistance <= mobutils::GetMobSkillRange(PMobSkill, PMob, PTarget))
            {
                ...
                FaceTarget(PActionTarget->targid);
...
void CMobController::DoCombatTick(time_point tick)
{
    ...
    // Deaggro players(and trusts/pets) zoning or in cutscenes
    if (PTarget && PTarget->status == STATUS_CUTSCENE_ONLY || PTarget->status == STATUS_DISAPPEAR)
    {
