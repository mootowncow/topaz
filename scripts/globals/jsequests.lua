...
tpz.jsequest.onTrigger = function(player, npc, job)
    ...
    if isCorrectJob and isLevelCapped then
        ...
        local itemTwoName = string.gsub(itemTwo:getName(), '_', ' ');
            ...
            elseif (stage == 1) then
                player:PrintToPlayer("You are on Stage 1. I require a " .. itemOneName .. " and a ".. itemTwoName .. ".", 0, npcName)
            elseif (stage == 2) then
                player:PrintToPlayer("You are on Stage 2. I still require " .. GetMeritsRemainingJSE(player) .. " merit points.", 0, npcName)
            elseif (stage == 3) then
    ...
    if isCorrectJob and isLevelCapped then
        ...
        elseif (stage == 2) then
            ...
            if meritsToAdd < 300 then
                ...
                player:PrintToPlayer("You have turned in " .. GetTotalMeritsGivenJSE(player) .. " merits. I still require " .. GetMeritsRemainingJSE(player) .. " merits.", 0, npcName)
            else
