...
local function tryDoubleUp(mob, target)
    ...
    -- Get current rolls #
    local doubleEffect = mob:getStatusEffect(tpz.effect.DOUBLE_UP_CHANCE)
    local prev_roll = mob:getStatusEffect(doubleEffect:getSubPower())
    local roll = prev_roll:getSubPower()
    local job = doubleEffect:getTier()
    ...
    if canDoubleUp and prev_roll then
        for _, rolls in ipairs(rollData) do
            if prev_roll:getType() == rolls.Effect then
                if roll == rolls.Lucky then
                    lucky = true
                    break
                end
            end
...
function onMobFight(mob, target)
    ...
    for _, rolls in ipairs(rollData) do
        if mob:hasStatusEffect(rolls.Effect) then
            local effect = mob:getStatusEffect(rolls.Effect)
    ...
    for _, rolls in ipairs(rollData) do
        if not activeRollTypes[rolls.Effect] then
            table.insert(availableRolls, rolls.RollId)
        end
    end

    if mob:hasStatusEffect(tpz.effect.DOUBLE_UP_CHANCE) and hasValidRoll then
        tryDoubleUp(mob, target)
    else
