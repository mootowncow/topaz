...
function onEquip(pet)
    pet:addListener("AUTOMATON_ATTACHMENT_CHECK", "ATTACHMENT_DISRUPTOR", function(automaton, target)
        ...
        if 
            master and 
            master:countEffect(tpz.effect.DARK_MANEUVER) > 0 and
            automaton:getLocalVar("dispel") < VanadielTime() and
            target:hasStatusEffectByFlag(tpz.effectFlag.DISPELABLE) and
            (automaton:checkDistance(target) <= target:getMeleeRange())
        then
