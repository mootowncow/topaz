function onTrigger(player)
    ...
    if (targ ~= nil)  then
        if (targ:isPC() or targ:isMob() or targ:isPet() or targ:isTrust()) then
            ...
            player:PrintToPlayer(string.format("%s Stats 1: MJOB: %i, SJOB: %i, HP: %i, MP: %i, MH Dmg: %i, OH DMG: %i, Ranged Dmg: %i Delay: %i,", targ:getName(), targ:getMainJob(), targ:getSubJob(), targ:getMaxHP(), targ:getMaxMP(), targ:getWeaponDmg(), targ:getOffhandDmg(), targ:getRangedDmg(), targ:getDelay()))
