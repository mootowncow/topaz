---------------------------------------------------------------------------------------------------
-- func: addmobskilllistentry
-- desc: Adds a mobskill to the targets skill list
---------------------------------------------------------------------------------------------------
...
function error(player, msg)
    ...
    player:PrintToPlayer("!addmobskilllistentry {mob} <skill>")
end
...
function onTrigger(player, arg1, arg2)
    ...
    targ:addSkillListEntry(skill)
    local skillName = string.gsub(arg1, '_', ' ')
    skillName = utils.PunctuateString(skillName)
    player:PrintToPlayer(string.format("Added skill (%s) to [%d] %s's skill list [%i].", skillName, targ:getID(), MobName(targ), targ:getSkillList()))
end